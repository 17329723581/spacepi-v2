<template>
  <v-app>
    ...
    :style="topNavigationShrinkStatus?'background: #1E0A4D':'background:linear-gradient(to bottom, rgba(0,0,0, 1), rgba(255,255,255, 0))'">
      ...
      <div class="tabs">
        <a @click="$router.push('/')">
          <div class="tabs-line" :style="$route.path=='/'?'border-top: 2px solid rgba(255, 255, 255, 0.6);':''"></div>
          {{$t("home")}}
        </a>
      ...
      <div class="locale">
        <v-menu offset-y>
          <template v-slot:activator="{ on, attrs }">
            <v-btn tile class="locale-btn" height="85px" elevation color="rgba(255, 255, 255, 0)" dark v-bind="attrs" v-on="on">
              <div class="logo-img">
